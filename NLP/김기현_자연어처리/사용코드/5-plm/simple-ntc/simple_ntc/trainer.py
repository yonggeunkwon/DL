    ...
    def attach(train_engine, validation_engine, verbose=VERBOSE_BATCH_WISE):
        ...
        def attach_running_average(engine, metric_name):
            RunningAverage(output_transform=lambda x: x[metric_name]).attach(
                engine,  # engine에 해당 metric_name으로 등록
                metric_name,
            )
        ...
        if verbose >= VERBOSE_BATCH_WISE:  # progress bar 등록
            pbar = ProgressBar(bar_format=None, ncols=120)
            pbar.attach(train_engine, training_metric_names)

        # If the verbosity is set, statistics would be shown after each epoch.
        if verbose >= VERBOSE_EPOCH_WISE:
            @train_engine.on(Events.EPOCH_COMPLETED)  # train engine의 epoch이 끝났을 때 아래의 함수 실행
            def print_train_logs(engine):  # 매 epoch 끝날때마다 출력
